...
def get_average_cross_validation_accuracy(args):
	...
	if dense_layer_count == 2:
		classifier = [Dense(32, activation=active_fun), Dense(32, activation=active_fun), Dense(1, activation='softmax'),]
	...
	for train_index, test_index in folds.split(image_filenames, csv_y_data):
		...
		model.fit(x_train_data, to_categorical(y_train_data), epochs=5, batch_size=5, verbose=0)
...
def optimize_hyperparameters():
	...
	fmin(get_average_cross_validation_accuracy, feature_space, algo=tpe.suggest, max_evals=2)
	print("Best hyperparameters:", best_hyper_parameter_tuple)
	print("Best hyperparameter accuracy:", best_hyper_parameter_correctness)

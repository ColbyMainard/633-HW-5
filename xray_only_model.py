...
def accuracy_percent(y_expected, y_actual):
	...
	y_expected = y_expected.reshape(50, 1)
	#y_actual = y_actual.reshape(50, 2)
	y_actual = y_actual.reshape(50, 1)
	num = 0.0
	denom = 50.0
...
def get_average_cross_validation_accuracy(args):
	try:
		...
		if dense_layer_count == 2:
			classifier = [Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(1, activation='sigmoid'),]
		elif dense_layer_count == 3:
			classifier = [Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(1, activation='sigmoid'),]
		elif dense_layer_count == 4:
			classifier = [Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(1, activation='sigmoid'),]
		elif dense_layer_count == 5:
			classifier = [Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(1, activation='sigmoid'),]
		elif dense_layer_count == 6:
			classifier = [Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(32, activation=active_fun), Dropout(0.1), Dense(1, activation='sigmoid'),]
		elif dense_layer_count == 6:
...
def implement_optimum_model(parameter_tuple):
	...
	model = Sequential(feature+classifier)
	model.compile(optimizer=compiler_optimizer, loss='binary_crossentropy',metrics=['accuracy'],)
	#model_history = model.fit(x_data, to_categorical(y_data), epochs=10, batch_size=5)
	model_history = model.fit(x_data, y_data, epochs=10, batch_size=5)
...
if __name__ == "__main__":
	...
	print("Model summary:")
	print(model.summary())
	csv_x_data, csv_y_data = parse_csv_data("test.csv", False)
	raw_x_data = csv_x_data[:,0]

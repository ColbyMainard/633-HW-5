...
def generate_model(layer_count, optimizer, activator_fun, layer_type):
	model = Sequential()
	num_pixels = 600*600
	denom_val = 1
	model.add(Conv2D(32, kernel_size=(2,2), activation=activator_fun, input_shape=(600,600,0)))
...
def get_cross_validation_accuracy(model, epoch_count):
	...
	for train_index, test_index in folds.split(image_filenames, csv_y_data):
		...
		x_train_data = []
		for filename in raw_x_train_data:
			filename = os.path.join("resized_train", filename)
			x_train_data.append(cv2.imread(filename, 0))
		x_train_data = np.array(x_train_data)
		print("Training data shape =", np.shape(x_train_data))
		x_test_data = []
		for filename in raw_x_test_data:
			filename = os.path.join("resized_train", filename)
			x_test_data.append(cv2.imread(filename, 0))
...
def find_optimal_parameters_xray_only(args):
	...
	del case
	test_model = generate_model(layer_count, optimizer, activator_fun, layer_type)
	return 1 - get_cross_validation_accuracy(test_model, epoch_count)
